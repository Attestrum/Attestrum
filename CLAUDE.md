...
**Gate split.** `sources` catches unapproved git pins; `licenses` catches transitive deps whose SPDX license isn't in `deny.toml`'s `allow` list. Both run in the local pre-commit hook. `cargo deny check bans` and `cargo deny check advisories` run only in CI, not the local hook: `bans` is an active gate enforcing `deny.toml`'s `wildcards = "deny"` (its `[bans].deny` list is intentionally empty); `advisories` is CI-only because RUSTSEC index queries are slow and the alert set shifts as new advisories publish. The advisory gate is currently green — `RUSTSEC-2024-0436` and `RUSTSEC-2023-0071` are accepted on documented threat-model grounds in `deny.toml`'s `[advisories].ignore`. Full rationale lives in `docs/research/cargo-deny-gates-rationale.md`.
...
- A **training-data-provenance compliance tool.** It produces cryptographic, audit-ready evidence backing the EU AI Act Article 53(1)(d) training-content summary — provable corpus composition, inclusion / non-inclusion proofs. Valuable to any organization that publishes a training corpus.
...
- A two-sided market — the buyer is the corpus publisher, not the rightsholder asking "was my work used."
