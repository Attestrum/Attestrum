...
|---|---|---|---|---|
| `sigstore/cosign-installer` | `@v3` (major-version pin + explicit `cosign version --json` grep-and-exit-1 on non-v3.x drift inside the workflow) | 2026-05-24 | `.github/workflows/cosign-interop.yml` (Sprint 4 E4.5) | Installs `cosign v3.x` so the `cosign verify-blob-attestation --new-bundle-format` interop test can run against a real cosign binary on the runner. Belt-and-suspenders pin per parked-plan tactical decision D — `@v3` is the action major-version pin; the in-workflow `case "$git_version" in v3.*) ... esac` catches both installer regressions and any cosign minor-version drift outside the v3 line. Tighten the case match to `v3.0.*` (or a specific minor) if stricter drift catching becomes warranted. |
