...
last_verified: 43ae8b4 2026-05-29
...
**Cosign byte-equivalence**: shipped at Sprint 4 E4.5. The interop test at `crates/attestrum-attest/tests/cosign_interop.rs` builds an empty-corpus manifest through the production `attestrum_pipeline::build_corpus`, signs it via `attestrum_attest::sign` against the Sigstore public-good roots, self-verifies via `attestrum_attest::verify` as a sanity gate, then shells out to `cosign verify-blob-attestation --new-bundle-format` and asserts exit 0 + `Verified OK` in stderr. The test is `#[ignore]`d by default; the dedicated workflow at `.github/workflows/cosign-interop.yml` (push:main-only, NO pull_request) installs cosign via `sigstore/cosign-installer@v3` (the action's major-version pin; the cosign binary it installs is the latest stable, v2.5.2 at 2026-05-25 — cosign has not shipped v3.x yet, the parked-plan brief's assumption that it had was caught by the first CI run), gates the binary version with a `--json | jq .gitVersion` grep that accepts `v2.5+` or future `v3+`, exchanges the GHA OIDC token for a sigstore-audience token, and runs the ignored test under `--include-ignored`. The identity-regex argument is plucked from `SignedAttestation.identity` post-sign (`regex::escape` + `^…$` anchor) so the assertion reflects the real SAN extracted from the bundle's cert rather than a predicted GHA-workflow URL pattern.

**Predicate-family widening (model-binding/v0.1 promotion, Commit 3).** `verify.rs` now factors the crypto + identity + predicate-type-gate core into `verify_statement(req) -> VerifiedStatement` (predicate left untyped as `serde_json::Value`), so the proof + binding predicate families verify through the same path. `VerifyRequest` gained `expected_predicate_type: Option<&str>` (`None` ⇒ `TRAINING_CORPUS_PREDICATE_TYPE`, preserving every pre-binding caller). `verify()` is now a thin typed wrapper: it pins `expected_predicate_type` to training-corpus and runs the Exit-8 `TrainingCorpusPredicate` deserialize on top of `verify_statement`. The sequence below is unchanged for the training-corpus path; the only difference is that the predicate-type check at the `InTotoStatement` step (step 8 in code) now compares against the caller's expected URI rather than a hard-coded training-corpus constant. The signed `walk_chain` (attestrum-bind) consumes `verify_statement` to verify a `model-binding/v0.1` bundle.
...
- cosign interop Rust test + CI workflow update to install cosign (v2.5+ now, or any future v3+) and run `cosign verify-blob-attestation --new-bundle-format` against every CI-emitted bundle. See `crates/attestrum-attest/tests/cosign_interop.rs` + `.github/workflows/cosign-interop.yml`. First GREEN CI run gated on cosign-binary-version assertion being correct (the parked-plan brief assumed cosign v3 existed; it does not as of 2026-05-25 — the workflow accepts v2.5+ as the minimum that supports `--new-bundle-format`).
...
- Inclusion / non-inclusion predicate verify paths via the typed `verify()` (E4's `verify()` verifies `training-corpus/v0.3` only). **Update (Commit 3):** the lower-level `verify_statement()` now verifies *any* expected predicate type, including `model-binding/v0.1`; a typed `verify()`-style accessor for the proof predicates remains a follow-up.
