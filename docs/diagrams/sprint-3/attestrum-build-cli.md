...
Source of truth: `code` (Sprint 3 E5 implementation). This diagram models the **`build`** subcommand only; `crates/attestrum-cli/src/main.rs` is the shared `clap` command enum that also hosts the sibling subcommands (`sign`, `verify`, `prove`, `publish`, `bind` / `walk-chain`, `diff` (corpus-version delta; see `docs/diagrams/corpus-diff/diff-pipeline.md`), and — from the v1.1 fuzzy-index work — `index build` (nested subcommand; see `docs/diagrams/index/build-and-query.md`), `decontaminate` (benchmark-contamination scan; see `docs/diagrams/decontaminate/decontaminate.md`), and the read-only analysis subcommands `compose` (training-content summary; see `docs/diagrams/compose/compose.md`), `dedup` (near-duplicate rate; see `docs/diagrams/dedup/dedup.md`), and `remove` (two-manifest removal evidence; see `docs/diagrams/remove/remove.md`), each documented in its own diagram). This is the user-facing entry point that calls into `attestrum-pipeline::build_corpus` after loading a `corpus.toml` spec (BUILD-PLAN §8.3). The binary is named `attestrum` (configured via `[[bin]] name = "attestrum"` in `crates/attestrum-cli/Cargo.toml`), so the user types `attestrum build --corpus <path> --workspace <path>` rather than `attestrum-cli build ...`.
