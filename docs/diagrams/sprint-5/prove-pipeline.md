...
last_verified: 6d4bcd0 2026-05-26
...
**Branch state at E5** (this commit, fuzzy match + Document inline-fingerprint live): all six `ProofTarget` variants from PATH-A-BRIEF §2.2 now work end-to-end. The three fuzzy arms (`Iscc`, `Perceptual`, `Document`) use **CAS re-fingerprint at prove time** — `prove()` opens `opts.cas_root` as a `attestrum_cas::CasStore`, iterates manifest leaves, fetches each leaf's bytes by BLAKE3, runs `fingerprint_text` / `fingerprint_image` per `entry.modality`, and computes the relevant distance. Threshold + confidence values are hardcoded per PATH-A-BRIEF §2.2 (ISCC ≤ 4 → 0.95; Perceptual ≤ 6 of 64 → 0.85; MinHash ≥ 850_000 ppm → 0.80; exact 1.0). `ProofTarget::Document` MIME-detects (UTF-8 → text, `image::guess_format` → image, else InvalidManifest) and runs multi-mode — exact-BLAKE3 / SHA-256 first (works for image but not text per the PROTECTED text-normalization pipeline; text Documents reliably fall through to ISCC at distance 0). Non-image leaves silently skipped during Perceptual scan; non-text leaves silently skipped during MinHash scan; `Modality::{Audio, Video, Pdf, Other}` leaves silently skipped during all fuzzy scans. ProveOpts gains one new field: `cas_root: Option<PathBuf>`, REQUIRED when any fuzzy arm is invoked. The Mermaid nodes that flip from grey to green at this commit: `isccPath`, `perceptPath`, `docPath`, `fpDispatch`, `fpText`, `fpImage`, `docMulti`, `docMulti2`, `fuzzyScan`, `fuzzyDec`, `evIscc`, `evPercept`, `evMinhash`. The E2-E4-green nodes stay green. `fpErr` stays grey (only fires for Audio/Video/Pdf modality dispatch which is post-Sprint-5). HF / URL fetches (`hfFetch`, `urlFetch`) panic pending E7. The non-inclusion path (`nonInc`, `predNonIncl`, `stmtNI`) panics pending E6. CLI (`cliEntry`, `cliPrint`) stays grey pending E8.
