...
last_verified: 55cbc02 2026-05-26
...
**Branch state at E4** (this commit, MVP gate — DSSE-sign live): when `opts.sign=true`, `prove()` canonicalizes the in-toto Statement via `InTotoStatement::canonical_json`, calls `attestrum_attest::sign` (Sigstore Bundle v0.3 + DSSE + Fulcio + Rekor v1 `dsse@0.0.1`), writes the bundle to `<opts.workspace.or($PWD/.attestrum)>/prove/inclusion-proof.sigstore.json`, and returns `ProofArtifact { bundle_path: Some(_), ... }`. **The bundle verifies end-to-end via `cosign v3+ verify-blob-attestation --new-bundle-format` without Attestrum installed.** When `opts.sign=false`, E3 behavior is preserved verbatim (`bundle_path: None`, no network, no OIDC). Two additional predicate fields land at E4: `proof_generated_at` populated unconditionally from `opts.source_date_epoch` via `jiff::Timestamp::from_second(...).to_string()` (RFC 3339, deterministic); `corpus.attestation_digest` populated from the new `opts.corpus_bundle_path: Option<PathBuf>` field when present (via `attestrum_cas::stream_hash_path`), otherwise stays at the E2 zeros-hex placeholder. `proof_generator_identity` stays `None` even when signed — the bundle's leaf cert is the authoritative identity. The Mermaid nodes that flip from grey to green at this commit: `dsseSign`, `signedOut`, `errSign`. The E2/E3-green nodes (`target`, `manifest`, `opts`, `dispatch`, `resolve`, `exactB3`, `exactS256`, `bundleExact`, `localPq`, `loadIdx`, `matchQuery`, `matchDec`, `auditPath`, `predIncl`, `evidenceVariant`, `evExact`, `stmt`, `signCheck`, `unsignedOut`, `errMan`, `errAmb`) stay green. Fuzzy paths (`isccPath`, `perceptPath`, `docPath`, `docMulti*`, `fuzzyScan`, `fuzzyDec`, `evIscc`, `evPercept`, `evMinhash`) panic with `unimplemented!("S5-D2 E5+")` pending E5. HF / URL fetches (`hfFetch`, `urlFetch`) panic with `unimplemented!("S5-D2 E7")` pending E7. The non-inclusion path (`nonInc`, `predNonIncl`, `stmtNI`) panics with `unimplemented!("S5-D2 E6")` pending E6. CLI (`cliEntry`, `cliPrint`) stays grey pending E8.
...
flowchart TB
  ...
  class target,manifest,opts,dispatch,resolve,exactB3,exactS256,bundleExact,localPq,loadIdx,matchQuery,matchDec,auditPath,predIncl,evidenceVariant,evExact,stmt,signCheck,unsignedOut,dsseSign,signedOut,errMan,errAmb,errSign shipped
  class isccPath,perceptPath,docPath,fpDispatch,fpText,fpImage,fpErr,docMulti,docMulti2,hfFetch,urlFetch,fuzzyScan,fuzzyDec,nonInc,evIscc,evPercept,evMinhash,predNonIncl,stmtNI,cliEntry,cliPrint,errFp,errSrc,errMerk deferred
