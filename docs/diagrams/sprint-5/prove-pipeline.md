...
last_verified: 125d21c 2026-05-26
...
**Branch state at E3** (this commit, exact-match path live, audit-path real): `prove()` continues to resolve the E2 exact-hash dispatch arms, but now constructs `attestrum_merkle::MerkleTree::new(...)` once over the manifest's `document_id` column and uses it for both `corpus.merkle_root = tree.root()` AND `predicate.audit_path = tree.audit_path(leaf_index)?` (hex-encoded). The predicate is now **cryptographically self-contained**: external verifiers can re-derive the corpus's Merkle root from `predicate.{leaf_hash, leaf_index, tree_size, audit_path}` alone via `attestrum_merkle::verify_audit_path` — no manifest re-read required. Single-leaf trees correctly emit `audit_path == []` (the leaf hash IS the root per RFC 6962). The Mermaid node that flips from grey to green at this commit: `auditPath`. The E2-green nodes (`target`, `manifest`, `opts`, `dispatch`, `resolve`, `exactB3`, `exactS256`, `bundleExact`, `localPq`, `loadIdx`, `matchQuery`, `matchDec`, `predIncl`, `evidenceVariant`, `evExact`, `stmt`, `signCheck`, `unsignedOut`, `errMan`, `errAmb`) stay green. Fuzzy paths (`isccPath`, `perceptPath`, `docPath`, `docMulti*`, `fuzzyScan`, `fuzzyDec`, `evIscc`, `evPercept`, `evMinhash`) panic with `unimplemented!("S5-D2 E5+")` pending E5. HF / URL fetches (`hfFetch`, `urlFetch`) panic with `unimplemented!("S5-D2 E7")` pending E7. The non-inclusion path (`nonInc`, `predNonIncl`, `stmtNI`) panics with `unimplemented!("S5-D2 E6")` pending E6. DSSE-sign + signed-output (`dsseSign`, `signedOut`) and CLI (`cliEntry`, `cliPrint`) stay grey pending E4 and E8.
...
flowchart TB
  ...
  class target,manifest,opts,dispatch,resolve,exactB3,exactS256,bundleExact,localPq,loadIdx,matchQuery,matchDec,auditPath,predIncl,evidenceVariant,evExact,stmt,signCheck,unsignedOut,errMan,errAmb shipped
  class isccPath,perceptPath,docPath,fpDispatch,fpText,fpImage,fpErr,docMulti,docMulti2,hfFetch,urlFetch,fuzzyScan,fuzzyDec,nonInc,evIscc,evPercept,evMinhash,predNonIncl,stmtNI,dsseSign,signedOut,cliEntry,cliPrint,errFp,errSrc,errMerk,errSign deferred
