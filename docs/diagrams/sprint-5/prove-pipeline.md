...
**v1.1 addition (2026-06-06) — indexed fuzzy lookup fast-path.** The three fuzzy dispatchers (`dispatch_minhash` / `dispatch_perceptual` / `dispatch_iscc`) now consult a derived sidecar index (built by `attestrum index build`, crate `attestrum-index`) before falling back to the exhaustive scan-every-leaf path. When a sidecar exists at `<cas_root>/index/<kind>/v1.idx` and its `BINDING_ROOT` matches the manifest's Merkle root (recomputed exactly as `prove()` does), the dispatcher gathers LSH candidates and scores only those against their **persisted** signatures — no CAS read, no re-fingerprinting — collapsing the ~42 s/query scan to sub-second. The exact recheck (`minhash_jaccard_ppm ≥ 850000`, `min(phash,blockhash) ≤ 6`, `iscc_composite_distance ≤ 4`) and the emitted proof are **unchanged** — the index only narrows which candidates get scored, so a `--no-index` (force-exhaustive) run produces a byte-identical proof (verified in `tests/fuzzy_index.rs`). A missing / corrupt / stale (binding-mismatch) sidecar silently reverts to the exhaustive scan. `ProveOpts` gains one field — `no_index: bool` (the `--no-index` flag); the api-surface golden is unaffected (it tracks the struct name, not fields). Full build + query flows: `docs/diagrams/index/build-and-query.md`. **No PROTECTED change** (fingerprint normalization + the locked thresholds untouched); the index is discovery-grade and unsigned.
